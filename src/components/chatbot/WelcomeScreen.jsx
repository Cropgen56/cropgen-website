...
import { ArrowDownToLine, EllipsisVertical, X, History as HistoryIcon, CircleChevronRight } from "lucide-react";
...
export default function WelcomeScreen({ onClose, onContinue }) {
    ...
    return (
        <div className="relative w-[300px] h-[450px] sm:w-[400px] sm:h-[500px] rounded-2xl shadow-[0_0_30px_0_rgba(42,182,115,0.6)] bg-white overflow-hidden">
            <div className="bg-cover bg-center bg-no-repeat h-[280px] relative"
                style={{ backgroundImage: "url('/assets/image/comman/welcome-banner.svg')" }} >
            
                {/* Header */}
                <div className="flex justify-between items-center p-4 relative z-20">
                    <div className="flex items-center gap-2">
                        <EllipsisVertical size={18} color="#fff" className="cursor-pointer" onClick={() => setMenuOpen(!menuOpen)} />
                        <span className="flex items-center gap-1">
                            <Image
                                ...
                                height={25}
                            />
                            <span className="text-white text-[10px] font-semibold">CropGen</span>
                        </span>
                    </div>
                    <div className="flex items-center gap-3">
                        <X size={16} className="text-white cursor-pointer" onClick={onClose} />
                    </div>
                </div>
                ...
                <div className="absolute inset-0 flex flex-col justify-center items-start px-6 text-white font-semibold pointer-events-none">
                    ...
                    <p className="text-sm md:text-lg font-bold pointer-events-none">your AI + satellite farming guide.</p>
                </div>
            </div>


            {/* Ask Question card */}
            <div className="absolute bottom-[7.5rem] md:bottom-[11.5rem] left-1/2 transform -translate-x-1/2 w-[85%] bg-white p-3 rounded-xl shadow-lg cursor-pointer flex justify-between items-center z-30"
                onClick={onContinue} >
                <div>
                    <p className="font-bold text-base text-[#2AB673] mb-1"> Ask Question </p>
                    <p className="text-[11px] text-[#9A9898]"> Monitor crops, get accurate farm insights, and boost yields. </p>
                </div>
                <span className="text-xl"> <CircleChevronRight color="#2AB673" size={24} /> </span>
            </div>
            ...
            <div className="absolute bottom-0 left-0 w-full flex justify-around py-4">
                ...
                <button className="flex flex-col items-center cursor-pointer group"
                    ...
                    />
                    <p className="text-sm font-bold text-[#9A9898] group-hover:text-[#2AB673] transition-all duration-500 ease-in-out">Messages</p>
                </button>
            </div>
        </div>
    );

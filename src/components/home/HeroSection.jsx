...
const HeroSection = () => {
  ...
  const imagesToWait = features.length + 1; // features + dashboard

  useEffect(() => {
    if (loadedImages >= imagesToWait) {
      const timer = setTimeout(() => setIsLoading(false), 200); // short fade
      return () => clearTimeout(timer);
    }

    const fallback = setTimeout(() => setIsLoading(false), 1000); // 1s max wait
    ...
    <section className="relative flex flex-col md:flex-row justify-between bg-white pb-7 md:pb-10">
      ...
      >
        <h1 className="text-xl md:text-3xl lg:text-4xl font-bold text-gray-900 leading-tight">
          ...
          <span>Crop Monitoring & </span> Advisory System for Precision Agriculture
        </h1>

        <p className="text-gray-700 text-sm md:text-base">
          CropGen uses AI , remote sensing and satellite crop monitoring to deliver real-time , precision agriculture solution for Indian farmers, agribusinesses , and researchers.
        </p>
      ...
      >
        ...
        <div className="relative w-full max-w-md md:max-w-lg lg:max-w-xl z-10 flex justify-start">
          ...
          {/* decorative images, load in background, no skeleton wait */}
          <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="absolute top-14 right-0 sm:top-20 sm:right-[-20px] w-1/3 md:w-1/3">
            <Image src="/assets/image/home/doughnut.png" alt="Agricultural Data Chart" width={200} height={120} className="w-full" />
          </motion.div>

          <motion.div animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="absolute bottom-0 right-0 sm:bottom-[10px] sm:right-[30px] w-1/2 md:w-1/2">
            <Image src="/assets/image/home/soil-chart.png" alt="Soil Analysis Graph" width={300} height={300} className="w-full" />
          </motion.div>
        </div>
      </motion.div>
    </section>
